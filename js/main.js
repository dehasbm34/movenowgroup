...
(function () {
  ...
  /* ── Language Toggle (auto-detect + 10 languages) ─────────── */
  function initLangToggle() {

    /* 1. Detect language -------------------------------------- */
    var browserRaw = (navigator.language || navigator.userLanguage || 'en')
                       .split('-')[0].toLowerCase();
    var langMap    = (window.LANG_MAP) || {};
    var detected   = langMap[browserRaw] || 'en';
    var stored     = localStorage.getItem('lang');
    var initial    = stored || detected;

    /* 2. Build toggle buttons from SUPPORTED_LANGS if present - */
    var container = document.getElementById('langToggle');
    if (container && window.SUPPORTED_LANGS) {
      container.innerHTML = '';
      window.SUPPORTED_LANGS.forEach(function (l) {
        var b = document.createElement('button');
        b.className   = 'lang-btn' + (l.code === initial ? ' active' : '');
    ...
    /* Legacy buttons (pages without dynamic rebuild) */
    ...
    function applyLang(lang) {
      ...
      var isRtl = ['ar', 'fa', 'ku'].indexOf(lang) !== -1;
      ...
      /* A) data-en / data-ar / data-fa legacy attributes */
      document.querySelectorAll('[data-en]').forEach(function (el) {
        var txt = el.dataset[lang] || el.dataset.en;
        if (txt !== undefined) el.textContent = txt;
      });

      /* B) data-i18n key lookup */
      ...
      /* C) Auto-translate nav + buttons by English text value  */
      if (lang !== 'en' && Object.keys(dict).length) {
        var selectors = [
          ...
          '.btn--outline:not([type="submit"])',
          '.emergency-bar__cta'
        ];
        document.querySelectorAll(selectors.join(',')).forEach(function (el) {
          var src = (el.dataset.en || el.textContent || '').trim();
          // Strip emoji prefix (e.g. "✈ Private Jets" → "Private Jets")
          var key = src.replace(/^[\u{1F300}-\u{1FAFF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\s]+/u, '').trim();
          if (dict[key])     { el.textContent = dict[key]; return; }
          if (dict[src])     { el.textContent = dict[src]; }
        });

        /* Emergency bar text */
        var barSpan = document.querySelector('.emergency-bar > span');
        if (barSpan) {
          var barKey = '⚡ 24/7 Emergency Line — If you are in immediate danger, call now';
          if (dict[barKey]) barSpan.textContent = dict[barKey];
        }
      }

      /* D) Placeholder translations */
      document.querySelectorAll('[data-ph-en]').forEach(function (el) {
        var ph = el.getAttribute('data-ph-' + lang) || el.getAttribute('data-ph-en');
        if (ph) el.placeholder = ph;
      });

      /* E) Update typewriter phrase list if running */
      if (window._typewriterSetLang) window._typewriterSetLang(lang);
    }

    /* Expose for external use (e.g. form pages) */
